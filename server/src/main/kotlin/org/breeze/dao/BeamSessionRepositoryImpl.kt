...
import org.breeze.dao.DatabaseFactory.dbQuery
...
class BeamSessionRepositoryImpl : BeamSessionRepository {
    ...
    override suspend fun beamSession(id: String): BeamSession? = dbQuery {
        BeamSessions.select { BeamSessions.id.eq(id) }.map(::resultRowToArticle).singleOrNull()
    }

    override suspend fun addNewBeamSession(
            id: String,
    ...
    override suspend fun editBeamSession(
            id: String,
            type: BeamSessionContentType,
            content: String
    ) = dbQuery {
    ...
    override suspend fun deleteAllBeamSessions(): Boolean = dbQuery { BeamSessions.deleteAll() > 0 }
}
